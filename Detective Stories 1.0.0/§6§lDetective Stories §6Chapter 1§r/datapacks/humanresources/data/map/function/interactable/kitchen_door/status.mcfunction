...
execute if entity @a[nbt={SelectedItem:{id:"minecraft:yellow_dye",components:{"minecraft:custom_model_data":{floats:[500f]}}}},tag=bartender_waiting] if entity @a[tag=language.chinese] run function map:dialogue/init_text {text:"我现在暂时用不上这把钥匙。",total_symbols:13,time:1,state:2,next_dialogue:"none",db_visual:1}
