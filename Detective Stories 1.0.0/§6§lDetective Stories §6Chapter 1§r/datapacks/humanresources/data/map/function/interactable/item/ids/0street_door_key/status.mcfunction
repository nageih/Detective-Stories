...
# Функция задержки
tag @s add cooldown
schedule function map:interactable/item/ids/0street_door_key/cooldown 10t
...
execute if entity @s[tag=!nah,nbt={interaction:{}}] run function map:interactable/item/ids/0street_door_key/pickup

# Если предмет можно поднять, и игрок нажал лкм, то предмет будет положен обратно
execute if entity @s[nbt={attack:{}}] run function map:interactable/item/ids/0street_door_key/remove
...
execute if score @s interactableStatus matches 1 run function map:interactable/item/ids/0street_door_key/show
execute if score @s interactableStatus matches 2 run function map:interactable/item/ids/0street_door_key/remove
