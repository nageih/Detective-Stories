# Достижение
execute unless score #music_advancement_song5 status matches 1 run scoreboard players set #music_advancement_song5 status 1
...
execute unless score #streamermode status matches 1 at @e[tag=aj.radio.root] run playsound minecraft:map.music.radio.lastchristmas master @a ~ ~ ~ 1 1
execute unless score #streamermode status matches 1 run schedule function map:interactable/radio/random 265s
