# Достижение
execute unless score #music_advancement_song12 status matches 1 run scoreboard players set #music_advancement_song12 status 1
...
execute unless score #streamermode status matches 1 at @e[tag=aj.radio.root] run playsound minecraft:map.music.radio.eminem_ncs master @a ~ ~ ~ 1 1
