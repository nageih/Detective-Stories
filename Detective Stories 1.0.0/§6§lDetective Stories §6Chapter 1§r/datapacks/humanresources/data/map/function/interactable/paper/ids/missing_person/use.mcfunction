...
function map:interactable/paper/ids/teleport {id:missing_person}

# !=Бумажка в лежачем положении
data merge entity @e[tag=missing_person,tag=interactable.prop.paper,type=item_display,limit=1] {transformation:{left_rotation:[0f,0.0f,0.0f,-0.01f]},start_interpolation:0}
data merge entity @e[tag=missing_person,tag=interactable.prop.paper,type=item_display,limit=1] {billboard:"center"}

# Бумага светится
data merge entity @e[tag=missing_person,tag=interactable.prop.paper,type=item_display,limit=1] {brightness:{sky:15,block:15}}
...
function map:interactable/paper/ids/missing_person/player_check